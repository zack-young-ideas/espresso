...
class Settings {
  ...
  constructor(settingsFilename) {
    ...
    if (this.filename) {
      try {
        const fileSettings = require(settingsFilename);
  ...
  get databaseUri() {
    ...
    */
    const containsNull = Object.values(this.database).some((value) => {
      return (value === null)
    });
    if (containsNull) {
      return null;
  ...
  async write() {
    ...
    output += '};';
    await writeFile(this.filename, output, { encoding: 'utf8' });
  }

  async updateDatabase(formObject) {
